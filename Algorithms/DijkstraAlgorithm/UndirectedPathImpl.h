...
/*
template <typename T, template <typename> class Vertex>
int findShortestPathInUndirectedGraph(int numberFrom, int numberTo, UndirectedGraph<T, Vertex>* graph) {

    MutableSegmentedDeque<Vertex<T>*>* allVertexes = graph->getAllVertices();

    Vertex<T>* currentVertex = graph->findVertex(numberFrom);
    Vertex<T>* finishVertex = graph->findVertex(numberTo);

    currentVertex->setShortestPath(0);
    currentVertex->setShortestPathOfVertices();
    
    MutableSegmentedDeque<Vertex<T>*>* allConnectedVertexes;
    int quantityOfSubmittedVertexes = 0;
    while (true) {
        allConnectedVertexes = currentVertex->getAllConnectedVertices();

        Vertex<T>* connectedVertex;
        Edge<T>* connectingEdge;
        for (int i = 0; i < allConnectedVertexes->getLength(); ++i) {
            connectedVertex = (*allConnectedVertexes)[i];
            if (connectedVertex->isVertexUsed()) {
                continue;
            }
            connectingEdge = currentVertex->findEdge(connectedVertex);
            if (connectedVertex->getShortestPath() > currentVertex->getShortestPath() + connectingEdge->getWeight()) {
                connectedVertex->setShortestPath(currentVertex->getShortestPath() + connectingEdge->getWeight());
                connectedVertex->setShortestPathOfVertices(currentVertex);
            }
        }

        currentVertex->setUsed();
        if (++quantityOfSubmittedVertexes == allVertexes->getLength()) {
            break;
        }
        
        int minShortestPath = 1000000000;
        for (int i = 0; i < allVertexes->getLength(); ++i) {

            if (allVertexes->get(i)->getShortestPath() < minShortestPath && !(allVertexes->get(i)->isVertexUsed())) {
                currentVertex = allVertexes->get(i);
                minShortestPath = currentVertex->getShortestPath();
            }
        }
    }
    return finishVertex->getShortestPath();
}
*/